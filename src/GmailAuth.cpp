...
bool GmailAuth::loadClientSecret() {
    ...
    try {
        ...
        clientId = installed["client_id"].get<string>();
        clientSecret = installed["client_secret"].get<string>();
        ...
        logger.log(LOG_ERROR, "Failed to parse client secret: " + string(e.what()));
        return false;
    }
}
string GmailAuth::urlEncode(const string value) {
...
string GmailAuth::makeTokenRequest(const string& postData) {
    ...
    curl_easy_setopt(curl, CURLOPT_CAINFO, "libs/curl/bin/curl-ca-bundle.crt");    
    struct curl_slist* headers = nullptr;
    headers = curl_slist_append(headers, "Content-Type: application/x-www-form-urlencoded");;
    curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
    CURLcode res = curl_easy_perform(curl);
    if (res != CURLE_OK) {
        logger.log(LOG_ERROR, "Curl request Failed: " + string(curl_easy_strerror(res)));
    ...
    logger.log(LOG_INFO, "HTTP Status: " + to_string(httpCode));
    curl_slist_free_all(headers);
    curl_easy_cleanup(curl);
    if (httpCode != 200) {
...
bool GmailAuth::exchangeCodeForTokens(const string& code) {
    ...
    string response = makeTokenRequest(postData.str());
    if (response.empty()) {
    ...
    try {
        ...
        if (j.contains("error")) {
            logger.log(LOG_ERROR, "Token Exchange Failed" + j["error"].get<string>());
            if (j.contains("error_description")) {
                logger.log(LOG_ERROR, "     " + j["error_description"].get<string>());
            }
            return false;
        }
        ...
        accessToken = j["access_token"].get<string>();
        ...
        logger.log(LOG_INFO, "Expires in (s): " + to_string(expiresIn));
        return true;
    } catch (const json::exception& e) {
        logger.log(LOG_ERROR, "Failed to parse token response: " + string(e.what()));
        logger.log(LOG_ERROR, "Response: " + response);
        return false;
    }
}
...
bool GmailAuth::loadTokens() {
    ...
    try {
        ...
        if (secondsRemaining > 0){
            logger.log(LOG_INFO, "Token valid for (s) " + to_string(secondsRemaining));
        } else {
            logger.log(LOG_WARNING, "Token expired (s) " + to_string((-secondsRemaining)));
        }
        return true;
    } catch(const json::exception& e) {
        logger.log(LOG_ERROR, "Failed to parse token cache: " + string(e.what()));
        return false;
    }
}
bool GmailAuth::refreshToken() {
    ...
    try {
        json j = json::parse(response);
        if (j.contains("error")) {
            logger.log(LOG_ERROR, "Token Refresh failed:" + j["error"].get<string>());
            return false;            
        }
        ...
        logger.log(LOG_ERROR,"Failed to parse refresh response: " + string(e.what()));
        return false;
    }
}
